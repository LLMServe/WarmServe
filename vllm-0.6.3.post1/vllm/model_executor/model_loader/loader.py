...
def substitue_model_parameters(model, new_tensors: Dict[str, torch.Tensor]):
    # Replace all parameters in the model to new_tensors
    for name, param in model.named_parameters():
        if name in new_tensors:
            new_param = torch.nn.Parameter(new_tensors[name], requires_grad=False)
            ...
            raise ValueError(f"Error: cannot find weight {name} in given GPU tensors.")
...
class DefaultModelLoader(BaseModelLoader):
    ...
    def load_model(self, *, model_config: ModelConfig,
        ...
        with set_default_torch_dtype(model_config.dtype):
            ...
            if loaded_tensors is not None:
                substitue_model_parameters(model, loaded_tensors)
                model = model.to(torch.device(device_config.device))
            else:
