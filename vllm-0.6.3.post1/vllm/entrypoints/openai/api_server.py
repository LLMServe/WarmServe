...
time1 = time.time()
print(f"ApiServer: import initial libraries time cost = {'%.2f' % (time1 - global_stime)} seconds")
...
time2 = time.time()
print(f"ApiServer: import vllm time cost = {'%.2f' % (time2 - time1)} seconds")

# Inspect Llama model to import llama in prior
from vllm.model_executor.models.registry import ModelRegistry
ModelRegistry.inspect_model_cls("LlamaForCausalLM")
...
time3 = time.time()
print(f"ApiServer: import llama time cost = {'%.2f' % (time3 - time2)} seconds")
print(f"ApiServer: import library total time cost = {'%.2f' % (time3 - global_stime)} seconds")

if int(os.getenv("VLLM_PREWARM_ENGINE", "0")) == 1:
    ray.init(namespace="prewarm")
    prewarm_manager = ray.get_actor("prewarm_manager", namespace="prewarm")
time4 = time.time()
print(f"ApiServer: Get prewarm_manager actor time cost = {'%.2f' % (time4 - time3)} seconds")

tokenizer_path = os.getenv("TOKENIZER_PATH", None)
if tokenizer_path:
    print("Since all models use the same tokenizer, we pre-init tokenizer to speed up cold starts.")
    import builtins
    from transformers import AutoTokenizer
    builtins.initialized_tokenizer = AutoTokenizer.from_pretrained(tokenizer_path, truncation_side='left')

time5 = time.time()
print(f"ApiServer: Pre-init tokenizer time cost = {'%.2f' % (time5 - time4)} seconds")
...
async def build_async_engine_client(
    ...
    end_arg_time = time.time()
    print(f"Obtain engine args time cost = {'%.2f' % (end_arg_time - start_build_time)} seconds")

    async with build_async_engine_client_from_engine_args(
            engine_args, args.disable_frontend_multiprocessing) as engine:
        logger.debug(f"Create_engine time cost = {'%.2f' % (time.time() - end_arg_time)} seconds")
        yield engine
...
async def run_server(args, **uvicorn_kwargs) -> None:
    ...
    async with build_async_engine_client(args) as engine_client:
        ...
        time1 = time.time()
        logger.debug(f"Build engine and app time cost = {'%.2f' % (time1 - start_build_time)} seconds")
        logger.debug(f"Start serving at {'%.2f' % (time1 - global_stime)} seconds")
...
if __name__ == "__main__":
    ...
    # Blocker for prewarming LLMEngine
    if int(os.getenv("VLLM_PREWARM_ENGINE", "0")) == 1:
        print("READY%", flush=True)
        # Get arguments
        args_str = sys.stdin.readline().strip()
        cur_time = time.time()
        print(f"ApiServer: wait for starting up time cost = {'%.2f' % (cur_time - time5)} seconds")
        time5 = cur_time
        global_stime = cur_time
    ...
    time6 = time.time()
    print(f"ApiServer: parse args time cost = {'%.2f' % (time6 - time5)} seconds")
