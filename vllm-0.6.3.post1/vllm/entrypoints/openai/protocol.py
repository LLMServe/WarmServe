...
class ChatCompletionRequest(OpenAIBaseModel):
    ...
    chat_template: Optional[str] = Field(
        default="{% for message in messages %}{% if message['role'] == 'user' %}{% if loop.first and messages[0]['role'] == 'system' %}{{ '[INST] <<SYS>>\\n' + messages[0]['content'] + '\\n<</SYS>>\\n\\n' }}{% else %}{{ '[INST] ' + message['content'] + ' [/INST]' }}{% endif %}{% elif message['role'] == 'assistant' %}{{ ' ' + message['content'] + ' ' }}{% endif %}{% endfor %}",
        description=(
    ...
    def to_sampling_params(self, default_max_tokens: int) -> SamplingParams:
        ...
        if all(getattr(guided_decoding, attr, None) is None for attr in vars(guided_decoding)):
