...
def gpu_p2p_access_check(src: int, tgt: int, world_name: str) -> bool:
    ...
    is_distributed = dist.is_initialized(name=world_name)
    ...
    if ((not is_distributed or get_world_group(world_name).local_rank == 0)
    ...
    if is_distributed:
        get_world_group(world_name).barrier()
