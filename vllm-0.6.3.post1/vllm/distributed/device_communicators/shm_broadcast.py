...
class MessageQueue:
    ...
    def create_from_process_group(pg: ProcessGroup,
                                  ...
                                  max_chunks,
                                  writer_rank=0,
                                  world_name="") -> "MessageQueue":
        group_rank = dist.get_rank(pg, world_name)
        group_world_size = dist.get_world_size(pg, world_name)
        global_ranks = dist.get_process_group_ranks(pg, world_name)

        from vllm.distributed.parallel_state import in_the_same_node_as
        status = in_the_same_node_as(pg, source_rank=writer_rank, world_name=world_name)
