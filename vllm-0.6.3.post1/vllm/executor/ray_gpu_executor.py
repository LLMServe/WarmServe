...
class RayGPUExecutor(DistributedGPUExecutor):
    ...
    def _init_executor(self) -> None:
        ...
        self.warmup_device = envs.VLLM_WARMUP_DEVICE
        ...
        logger.debug(f"init_executor time cost = {'%.2f' % (time.time() - stime)} seconds")
    ...
    def _get_worker_wrapper_args(self) -> Dict[str, Any]:
        ...
        return dict(
            ...
            use_ray_compiled_dag=self.use_ray_compiled_dag,
    ...
    def _init_workers_ray(self, placement_group: "PlacementGroup",
        ...
        logger.info("warmup_device: %s", self.warmup_device)

        # Create the workers.
        driver_ip = get_ip()
        if self.warmup_device:
            if not self.use_ray_spmd_worker:
                raise ValueError("VLLM_WARMUP_DEVICE=1 requires VLLM_USE_RAY_SPMD_WORKER=1 to avoid initializing the driver worker's device twice.")
            prewarm_manager = ray.get_actor("prewarm_manager", namespace="prewarm")
            current_node_id = ray.get_runtime_context().get_node_id()
            prewarm_engine_id = int(os.getenv("VLLM_PREWARM_ENGINE_ID", "-1"))
            workers, prewarm_id = ray.get(prewarm_manager.get_workers.remote(current_node_id, self.parallel_config, self.model_config.model, prewarm_engine_id))
            while not workers:
                logger.warning("Retry getting workers...")
                time.sleep(1)
                workers, prewarm_id = ray.get(prewarm_manager.get_workers.remote(current_node_id, self.parallel_config, self.model_config.model, prewarm_engine_id))
            ...
            target_bundle_ids = os.getenv("TARGET_BUNDLE_IDS", None)
            if target_bundle_ids:
                target_bundle_ids = target_bundle_ids.split(',')
            for bundle_id, bundle in enumerate(placement_group.bundle_specs):
                if not bundle.get("GPU", 0):
                    continue
                if target_bundle_ids is not None and str(bundle_id) not in target_bundle_ids:
                    continue
        ...
        time_1 = time.time()
        logger.debug(f"get workers time cost = {'%.2f' % (time_1 - stime)} seconds")
        ...
        # Sort newly initialized workers
        if not self.warmup_device:
        ...
        if not self.use_ray_spmd_worker and n_nodes != n_ips:
            raise RuntimeError(
        ...
        time_2 = time.time()
        logger.debug(f"get worker metadata time cost = {'%.2f' % (time_2 - time_1)} seconds")
        ...
        all_args_to_update_environment_variables = [({
            **({
            ...
            } if not self.warmup_device else {}),
            "VLLM_INSTANCE_ID":
            VLLM_INSTANCE_ID,
            "VLLM_TRACE_FUNCTION":
        ...
        time_3 = time.time()
        logger.debug(f"init_worker time cost = {'%.2f' % (time_3 - time_2)} seconds")

        self._run_workers("init_device")

        time_4 = time.time()
        logger.debug(f"init_device time cost = {'%.2f' % (time_4 - time_3)} seconds")
        ...
        time_5 = time.time()
        logger.debug(f"load_model time cost = {'%.2f' % (time_5 - time_4)} seconds")
    ...
    def execute_model(
        ...
        # Execute the request by explicitly calling worker methods
        serialized_input = self.input_encoder.encode(execute_model_req)
        output = serialized_input
        for pp_rank, tp_group in enumerate(self.pp_tp_workers):
            output_futures = [
                worker.execute_model_spmd.remote(output)
                for i, worker in enumerate(tp_group)
            ]
            # NOTE: Once the first worker finished, all other workers in the TP group has finished.
            # We only need to get the output from the first worker.
            output = ray.get(output_futures[0])
