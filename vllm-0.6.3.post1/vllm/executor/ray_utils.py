...
try:
    ...
    class RayWorkerWrapper(WorkerWrapperBase):
        ...
        def __init__(self, *args, **kwargs) -> None:
            self.use_ray_complied_dag = kwargs.pop("use_ray_compiled_dag", True)
            super().__init__(*args, **kwargs)

            if self.use_ray_complied_dag:
        ...
        def execute_model_spmd(
            ...
            if isinstance(req_or_tuple, bytes):
                ...
                serialized_req, intermediate_tensors, execute_model_req = req_or_tuple
...
def initialize_ray_cluster(
    ...
    if envs.VLLM_WARMUP_DEVICE:
        # No need to create placement group because workers have been created.
        return

    device_str = "GPU" if not current_platform.is_tpu() else "TPU"
    # Create placement group for worker processes
    skip_verify = True if int(os.getenv("SKIP_RAY_VERIFY", "0")) == 1 else False
    if skip_verify:
        current_placement_group = ray.util.get_placement_group("prewarming_group")
    else:
