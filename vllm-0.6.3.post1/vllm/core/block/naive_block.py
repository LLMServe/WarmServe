...
class NaiveBlockAllocator(BlockAllocator):
    ...
    def __init__(
        ...
        # KV Prewarming. The shared memory will be set by AsyncLLMEngine
        self.kv_shm = None
        self.cur_reserved_blocks = 0
    ...
    def _free_block_id(self, block: Block) -> None:
        ...
        if refcount == 0:
            ...
            if self.kv_shm:
                target_free_blocks = struct.unpack("i", self.kv_shm.buf[:4])[0]
                while len(self._free_block_indices) > target_free_blocks:
                    # Reserve this block
                    block_id = self._free_block_indices.pop()
                    self.kv_shm.buf[8 + self.cur_reserved_blocks * 4: 12 + self.cur_reserved_blocks * 4] = struct.pack("i", block_id)
                    self.cur_reserved_blocks += 1
                self.kv_shm.buf[4:8] = struct.pack("i", self.cur_reserved_blocks)
